...
async function sendNotifications(data: Record<string, string>) {
  ...
  const participantSubject = data.language === "tr" ? "Avrupa Yerel ve Bölgesel Yönetim Birlikleri Zirvesi katılım yanıtınız" : "Your RSVP for the European Summit of Associations of Local and Regional Authorities";
  const participantHtml = data.language === "tr" ? `<p>Sayın ${escapeHtml(data.firstName)} ${escapeHtml(data.lastName)},</p><p>Avrupa Yerel ve Bölgesel Yönetim Birlikleri Zirvesi için yanıtınız kaydedilmiştir.</p><p><strong>Katılım durumu:</strong> ${responseText}</p><p>13 Ekim 2026 · 14.00–18.10<br>CEMR Toplantı Salonu, 1. Kat<br>Square de Meeûs 1, 1000 Brüksel</p><p>Türkiye Belediyeler Birliği</p>` : `<p>Dear ${escapeHtml(data.firstName)} ${escapeHtml(data.lastName)},</p><p>Your response for the European Summit of Associations of Local and Regional Authorities has been recorded.</p><p><strong>Attendance status:</strong> ${responseText}</p><p>13 October 2026 · 14:00–18:10<br>CEMR Meeting Room, 1st Floor<br>Square de Meeûs 1, 1000 Brussels</p><p>Union of Municipalities of Türkiye</p>`;
